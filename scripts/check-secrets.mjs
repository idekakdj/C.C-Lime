...
const found=[];
for(const name of names){if(!fs.existsSync(name)||!fs.statSync(name).isFile()||fs.statSync(name).size>10*1024*1024)continue;const text=fs.readFileSync(name,'utf8');if(signatures.some(pattern=>pattern.test(text))||privateEntries.some(value=>text.includes(value)))found.push(name);}
